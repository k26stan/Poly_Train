...
for ( run in RUNS.del ) {
	...
	R2UP.9.del[[run]][,"TR"] <- DEL[[run]]$R2_9[ VARS.9.del[[run]], "TR" ] - DEL[[run]]$R2_9[ (length(COVS.del)):(length(COVS.acr)+len.9-1), "TR" ]
	R2UP.9.del[[run]][,"TS"] <- DEL[[run]]$R2_9[ VARS.9.del[[run]], "TS" ] - DEL[[run]]$R2_9[ (length(COVS.del)):(length(COVS.acr)+len.9-1), "TS" ]
	R2UP.1.del[[run]][,"TR"] <- DEL[[run]]$R2_1[ VARS.1.del[[run]], "TR" ] - DEL[[run]]$R2_1[ (length(COVS.del)):(length(COVS.acr)+len.1-1), "TR" ]
	R2UP.1.del[[run]][,"TS"] <- DEL[[run]]$R2_1[ VARS.1.del[[run]], "TS" ] - DEL[[run]]$R2_1[ (length(COVS.del)):(length(COVS.acr)+len.1-1), "TS" ]

}

## Compile Variant Ranks
 # Regression Model
ALL.VARS.9.del <- Reduce( union, VARS.9.del )
RANKS.9.del <- array( , c(length(ALL.VARS.9.del),length(RUNS.del)) )
rownames(RANKS.9.del) <- ALL.VARS.9.del
colnames(RANKS.9.del) <- RUNS.del
...
RANKS.1.del <- array( , c(length(ALL.VARS.1.del),length(RUNS.del)) )
rownames(RANKS.1.del) <- ALL.VARS.1.del
colnames(RANKS.1.del) <- RUNS.del
...
COMP.9.del <- data.frame( COUNT=COUNT.9.del, MEAN_RNK=round(MEAN.9.del,2), STAT=round(STAT.9.del,2), RANKS.9.del )
COMP.9.del <- COMP.9.del[order(COMP.9.del[,"STAT"]),]
COMP.1.del <- data.frame( COUNT=COUNT.1.del, MEAN_RNK=round(MEAN.1.del,2), STAT=round(STAT.1.del,2), RANKS.1.del )
COMP.1.del <- COMP.1.del[order(COMP.1.del[,"STAT"]),]

##################################################
## CONTINUOUS: ACR50 Results
RUNS.acr <- names(ACR)

## Set up variables to compile
 # List of variants in each moacr
...
RANKS.9.acr <- array( , c(length(ALL.VARS.9.acr),length(RUNS.acr)) )
rownames(RANKS.9.acr) <- ALL.VARS.9.acr
colnames(RANKS.9.acr) <- RUNS.acr
...
# RANKS.1.acr <- array( , c(length(ALL.VARS.1.acr),length(RUNS.acr)) )
# rownames(RANKS.1.acr) <- ALL.VARS.1.acr
# colnames(RANKS.1.acr) <- RUNS.acr
...
## Prioritize Variants
 # Count how many moacrs each variant appears in
...
# MEAN.1.acr <- apply( RANKS.1.acr, 1, mean, na.rm=T )
 # Take MEAN rank and divide by number of runs in which variant appears
 # This boosts variants that appear in multiple moacrs by giving a lower rank STAT
STAT.9.acr <- MIN.9.acr / COUNT.9.acr
# STAT.1.acr <- MEAN.1.acr / COUNT.1.acr

## Compile into single table
COMP.9.acr <- data.frame( COUNT=COUNT.9.acr, MEAN_RNK=round(MEAN.9.acr,2), STAT=round(STAT.9.acr,2), RANKS.9.acr )
COMP.9.acr <- COMP.9.acr[order(COMP.9.acr[,"STAT"]),]
# COMP.1.acr <- data.frame( COUNT=COUNT.1.acr, MEAN_RNK=round(MEAN.1.acr,2), STAT=round(STAT.1.acr,2), RANKS.1.acr )
